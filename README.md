# Horrible Launcher - API Documentation

**Здесь описаны все функции, классы с их методами и хуки которые предоставляет API лаунчера Horrible.**</br>
...
## Техническая справка
*Прочтите этот раздел перед написанием кода, чтобы избежать вылетов приложения.*

### 1. Движок и Синтаксис
* **JavaScript Движок:** **Rhino 1.7.7+** (Модифицированный).
* **Поддерживается (ES6 Features):**
    * ✅ Стрелочные функции: `const func = () => { ... }`
    * ✅ Переменные: `let`, `const`
    * ✅ Шаблонные строки: `` `Текст: ${переменная}` ``
    * ✅ Методы массивов: `.find()`, `.includes()`, `.map()`, `.filter()`
* **НЕ Поддерживается:**
    * ❌ `BigInt` (числа вида `100n`)
    * ❌ `async` / `await`
    * ❌ `class` (работает нестабильно, используйте функции-конструкторы)

### 2. Критические ограничения (Причины крашей)
В лаунчере действует строгая изоляция потоков. Нарушение этих правил приведет к **моментальному вылету**:
* ⛔ **Запрещены фоновые потоки:** Нельзя использовать `new java.lang.Thread()`, `java.util.Timer`, `android.os.Handler` или `Executors`.
* ⛔ **Запрещен доступ к UI из фона:** Попытка обратиться к буферу обмена или экрану до полной загрузки мира убьет процесс.
* **Правильное использование:** Весь код должен запускаться **синхронно** через систему [Module](#module) (по нажатию кнопки или переключателя в меню).

### 3. Пример безопасного шаблона скрипта
```javascript
// Создаем модуль в категории Other
const myModule = new Module("TestMod", false, false, "Other");

// Создаем кнопку действия
const myBtn = new ButtonSetting("Click Me", (view) => {
    // Весь код пишем здесь (это безопасно)
    if (typeof LocalPlayer !== "undefined" && LocalPlayer.isInGame()) {
        print(`Привет, ${LocalPlayer.getNameTag()}!`);
    } else {
        print("Зайдите в мир!");
    }
});
...
- `LocalPlayer.getPositionY();` - возвращает позицию по `y`.
...
- `MoveButton.SHIFT` - Кнопка приседания (Sneak). Игрок передвигается медленно, скрывает свой никнейм и не падает с края блоков. А еще тут можно заняться КЕКС.
